function Sidebar({ status, onNavigate }) {
  const menuItems = [
    {
      icon: (
    ...
    {
      icon: (
        <svg viewBox="0 -3 1030 1030" className="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
  ...
  return (
    <div className="sidebar" style={{ display: status ? 'flex' : 'none' }}>
      <div className="sidebar-title">Car Listings</div>
      <nav>
        <ul className="sidebar-nav">
          {menuItems.map((item, index) => (
            <li key={index} className="sidebar-nav-item" onClick={() => onNavigate && onNavigate(item)}>
