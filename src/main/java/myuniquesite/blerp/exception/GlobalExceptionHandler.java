 ...
 * (using the OutputStream) to prevent the conflict with Thymeleaf's use of the Writer.
 */
@ControllerAdvice
public class GlobalExceptionHandler {

    /**
     * Handles ResourceNotFoundException, returning a 404 Not Found response as structured JSON.
    ...
    /**
     * Catches all other generic exceptions and forces a structured JSON response (500 Internal Server Error).
     * * KEY FIX: By returning ResponseEntity and annotating with @ResponseBody, we guarantee 
     * that Spring uses the binary stream (OutputStream) to serialize the ApiError object to JSON.
     * This prevents the DispatcherServlet from trying to use the Thymeleaf View Resolver, 
    ...
    public ResponseEntity<ApiError> handleGenericException(Exception ex) {
        ...
        ApiError apiError = new ApiError(
            ...
            "An unexpected error occurred. Please check server logs for details. Exception: " + ex.getClass().getSimpleName()
        );
    ...
    public ResponseEntity<Map<String, String>> handleMethodAgumentNotValid(MethodArgumentNotValidException ex, Model model) {
